...
contract QueueBombPatchedTest is Test {
    ...
    function testTooManyIndicesReverts() public {
        ...
        // -----------------------------------------------------------------
        // Prove funds are NOT frozen: aggregate within limit and verify
        // -----------------------------------------------------------------
        uint256 preReceiver = legit.balanceOf(receiver);
        uint256 preBridge = legit.balanceOf(address(bridge));

        uint256 ok = bridge.MAX_AGGREGATED_WITHDRAWALS();
        uint256[] memory okIdx = new uint256[](ok);
        for (uint256 i = 0; i < ok; i++) {
            okIdx[i] = i;
        }
        console.log(
            "Aggregating",
            ok,
            "withdrawals post-revert - should succeed"
        );
        bridge.finaliseQueuedWithdrawalsAggregatedLimited(
            receiver,
            address(legit),
        ...
        assertEq(
            postReceiver - preReceiver,
            ok * 1 ether,
            "Receiver did not get expected funds"
        );
        assertEq(
            preBridge - postBridge,
            ok * 1 ether,
            "Bridge balance not debited correctly"
        );
        console.log("Funds successfully transferred - not frozen\n");
    }
